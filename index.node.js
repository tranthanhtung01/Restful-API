 import './src/server/utils/checkConnect_mongo'
...
if (!isProduction) {
  /* <- hot reload server-side code on development mode */
  const watcher = chokidar.watch('./src', {ignoreInitial: true});

  watcher.on('all', (event, filename) => {
    console.log(
      chalk.magenta('hot code reload'),
...
if (hydrateMode) {
  /* <- on hydrate-mode, instead of serving node.js */
  require('./src/server').hydrate();
} else {
  server.listen(port, () => {
    /* <- serve express as normal */
